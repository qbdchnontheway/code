...
data=read.table('pairwisepfs.txt',sep='\t',header=T)
...
forest(meta_res,
       layout = "BMJ",  # 使用BMJ近似格式
       col.diamond.fixed = "#045a8d",    # 固定效应菱形颜色
       col.diamond.random = "#bd0026",   # 随机效应菱形颜色
       col.square = "#2c7fb8",           # 研究方块颜色
       col.study = "black",              # 研究文字颜色
       col.inside = "black",             # 置信区间文字颜色
       xlim = c(0.1, 10),                # X轴范围
       xlab = "Hazard Ratio (HR)",       # X轴标签
       leftlabs = c("Study", "logHR", "SE"), # 左侧列标签
       rightlabs = c("HR", "95% CI"),    # 右侧列标签
       fontsize = 10,                    # 字体大小
       squaresize = 0.8,                 # 方块大小
       lty.fixed = 2,                    # 固定效应线型
       lty.random = 3,                   # 随机效应线型
       print.I2 = TRUE,                  # 显示I²
       print.tau2 = TRUE,                # 显示tau²
       print.p = TRUE,                   # 显示p值
       print.Q = TRUE,                   # 显示Q统计量
       test.overall.fixed = TRUE,        # 显示固定效应检验
       test.overall.random = TRUE)       # 显示随机效应检验


setwd('/Users/Bao-Dong Qin/Desktop/BRAF-CRC/1-data/first-ORR')
data=read.table('singlearm.txt',sep='\t',header=T)
#单个率的分析
